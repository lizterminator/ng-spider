...
$('#machine_tabs>li>a,#machine_tabs_>li>a').click(function() {
	...
	if (type === 'tomcat') {
		...
		tab_ids = ['pyinfo_tab_', 'seinfo_tab_', 'sedeploy_tab'];
	}
...
(function() {
	...
	function toggleMachine(e) {
		if (e.type === "click") {
			var ip = store.getItem('machine_ip');
			var port = store.getItem('machine_port');
			var machine_type = store.getItem('machine_type');
		} else {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.success) {
					...
					var p = document.getElementById(ip + '-' + port +'-'+machine_type);
					console.log(p);
